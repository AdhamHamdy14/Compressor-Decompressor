...
def count_frequencies(events: list) -> tuple:
    ...
    # This prevents allocating 286 slots for sparse data
    # (e.g., if only 'a' exists), ensuring only symbols present in the input
    # occupy space.
    lit_freq = {}
    dist_freq = {}
...
def generate_canonical_codes(sorted_lengths: list) -> dict:
    ...
    Returns:
        dict: A dictionary mapping each symbol to its binary string
              representation (e.g., "010").
    """
    ...
    for symbol, length in sorted_lengths:
        ...
        if length != 0:
            # Retrieve the current available integer code for this specific
            # bit length.
            code_value = next_code[length]

            # [MODIFICATION]: Convert the integer to a padded binary string.
            # Storing it directly as a string (e.g., "010") fixes the
            # BitWriter compatibility.
...
def generate_codes_from_array(lengths_array: list) -> dict:
    """
    Reconstructs Canonical Huffman codes directly from a
    fixed-size lengths array.

    This function acts as an adapter for the decompression phase. It takes the
    flat list of bit lengths read from the file header, filters out unused
    symbols, applies the strict canonical sorting rules, and delegates the
    actual code generation to the core Huffman engine.
    ...
    # 1. Filter out unused symbols (length 0) and map to
    # (symbol, length) tuples.
    # The 'enumerate' function elegantly uses the list index as
    # the symbol value.
    active_lengths = [
    ...
    # This precise ordering ensures the decompressor assigns the exact same
    # codes as the compressor.
    sorted_lengths = sorted(active_lengths, key=lambda x: (x[1], x[0]))

    # 3. Delegate to the core generation function using the perfectly
    # formatted data.
