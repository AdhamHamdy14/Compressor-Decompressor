...
class Node:
    def __init__(self,symbol,freq,left=None,right=None):
        self.symbol=symbol  # The actual symbol for leaves, or a dummy for internal nodes
        ...
        # Determine the minimum symbol in this subtree for deterministic tie-breaking
        if left is None and right is None:
            # Leaf node: the minimum symbol is its own symbol
            self.min_symbol=symbol
        else:
            # Internal node: the minimum symbol is the smallest between its left and right children
            self.min_symbol=min(left.min_symbol,right.min_symbol)

    # Magic method to define the "Less Than" (<) operator for the Priority Queue
...
def count_frequencies(events: list) -> tuple:
    ...
    while i<len(events):
        ...
        # Case 2: Length symbol (257-285)
            ...
            # c) Following a length symbol, the next integer must be a distance symbol
            dist_symbol=events[i]
            dist_freq[dist_symbol]=dist_freq.get(dist_symbol,0)+1
            i+=1

            # d) Skip the extra bits for the distance (if they exist as a Tuple)
            if i<len(events) and isinstance(events[i],tuple):
                i+=1
...
def build_huffman_tree(frequencies: dict):
    ...
    while len(priority_queue)>1:
        ...
        parent=Node('X',left_child.freq+right_child.freq,left_child,right_child)
...
def generate_canonical_codes(sorted_lengths: list) -> dict:
    ...
    for bits in range(1,max_length+1):
        ...
        # (previous code + number of symbols at the previous length) 
        # and shifting left by 1 (acting like moving down a floor, adding a 0 bit)
    ...
    for symbol,length in sorted_lengths:
        ...
        if length!=0:
            ...
            # Increment the code value for the next symbol that shares this bit length
            next_code[length]+=1
